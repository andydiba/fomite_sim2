...
void delete_queue(Queue* q){  
	...
	//free the queue structure itself  (not reusable!)
	free(q);
...
void insert_queue(Queue* q, void* dest, unsigned long index){
	//opposite to peek 
	unsigned long offset,pos;
	unsigned int k, nBytes;
	...
	char* dptr;
	 ...
	 if(index < q->count)
		...
		for(k=0;k<nBytes;k++)
		{
		     qptr[pos+k] = dptr[k];		//see peek_queue for difference
		}	  
  	
 	 }else{
		
            printf("\nWarning: (insert_queue) Index out of bounds or empty queue!");	
    	 }
}      
